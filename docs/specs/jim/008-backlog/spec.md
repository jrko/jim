...
status: approved
origin:
  - docs/brainstorms/20260331-backlog-skill.md
---
...
A `/jim:backlog` skill that scans docs/specs, docs/brainstorms, and docs/notes for deferred and out-of-scope work, consolidates related items, and produces a living `BACKLOG.md`. A post-build feedback loop keeps the file current as specs evolve.
...
- [ ] Output is a complete replacement of `BACKLOG.md` each run — no incremental appending
...
```mermaid
flowchart LR
    A[spec/plan/research/brainstorm/notes sources] --> B[Scan & Extract]
...
- **Scanning non-jim docs** — only `docs/specs`, `docs/brainstorms`, and `docs/notes` are scanned. Application-level TODOs, GitHub issues, or external trackers are not in scope.
