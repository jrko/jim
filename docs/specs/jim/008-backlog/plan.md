...
Create a `/jim:backlog` skill owned by the PM agent that scans docs/specs, docs/brainstorms, and docs/notes for deferred and out-of-scope work, consolidates related items, and writes `BACKLOG.md`. Integrate into the build completion gate using the same conditional pattern as the ARCHITECTURE.md feedback loop.
...
- **Chosen:** Add a step to the build completion gate that checks if `BACKLOG.md` exists and invokes `/jim:backlog` if so. File existence is the opt-in signal.
...
| Workflow docs | `WORKFLOW.md` | Update | Add `/jim:backlog` to command reference, artifacts table, agents table, PM skill composition example, and plugin directory tree |
...
```mermaid
flowchart TD
    INVOKE["/jim:backlog invoked"] --> SCAN["Glob + Grep specs, brainstorms, notes\nfor deferred items"]
    ...
    PRESENT -->|Approved| WRITE["Write BACKLOG.md"]
...
4. [x] Update `skills/build/SKILL.md` completion gate (section 5) to add a backlog feedback step after the architecture step. The new step: check if `BACKLOG.md` exists; if yes, invoke `/jim:backlog` to regenerate it; if no, skip. Insert between the existing arch step and the completion report. Renumber existing steps as needed.
   **Verify:** `grep -q 'BACKLOG.md' skills/build/SKILL.md && grep -q '/jim:backlog' skills/build/SKILL.md`

5. [x] Update `WORKFLOW.md` to document `/jim:backlog`. Add to: (1) Command Reference table — command, description, agent (`@jim:pm`), output (`BACKLOG.md`); (2) Project Artifacts table — Backlog artifact at `BACKLOG.md`; (3) Agents table — add `/jim:backlog` to `@jim:pm` "Used By" column; (4) PM agent skill composition example — add `backlog` to the `skills:` list; (5) Plugin Directory tree — add `backlog/` under the Strategic skills section with `SKILL.md` and `assets/backlog-template.md`.
   **Verify:** `grep -q '/jim:backlog' WORKFLOW.md && grep -q 'BACKLOG.md' WORKFLOW.md`
...
- Updating `ARCHITECTURE.md` — handled by the build's existing arch feedback loop, which will pick up the new skill directory.
