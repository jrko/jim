...
- **Desired State:** Skill and agent prose contains no literal default filenames for configurable paths — only `{path.*}` placeholders. A shared `skills/_shared/resolve-paths.md` preamble, referenced from every skill's step 1, instructs the agent to read `.jim/config.md`, validate it against `skills/_shared/config-schema.md`, and resolve placeholders for substitution at point of use. The placeholder syntax itself is the forcing function — `{path.*}` strings are not valid tool arguments, so the agent must resolve them before any tool call. Unknown keys or malformed values raise a hard error rather than falling through silently.
- **Affected Systems:**
...
- [ ] `skills/_shared/resolve-paths.md` exists and defines: (a) how the agent reads and validates `.jim/config.md`, (b) how `{path.*}` placeholders are resolved against the schema, (c) the rule that no placeholder may flow into a tool call unresolved.
...
- [ ] Every skill invocation runs the resolve-paths preamble before any filesystem-touching tool call. Placeholders (`{path.*}`, `{specs.*}`, `{workflow.*}`) are resolved internally and substituted at point of use; no literal placeholder string is ever passed to a `Write`, `Edit`, `Read`, `Glob`, or `Bash` argument.
