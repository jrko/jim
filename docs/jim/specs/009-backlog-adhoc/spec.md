...
The `/jim:backlog` skill currently rebuilds `BACKLOG.md` as a complete replacement each run, sourced entirely from scanning `docs/jim/` for deferred and out-of-scope work. This creates two related friction points:
...
flowchart LR
    ...
    C[docs/jim/ sources] --> D[Scan & consolidate]
