...
spec: "docs/jim/specs/009-backlog-adhoc/spec.md"
type: feature
status: complete
---
...
| Workflow doc | `docs/jim/WORKFLOW.md` | Update | Update the `/jim:backlog` command reference cell to mention the `add <description>` subcommand |

No changes to `agents/pm.md`, `skills/build/SKILL.md`, or `docs/jim/ARCHITECTURE.md`. The PM agent already owns `/jim:backlog`; the build completion gate already conditionally invokes `/jim:backlog` and the new behavior flows through transparently.
...
13. [x] Update `docs/jim/WORKFLOW.md` command-reference row for `/jim:backlog` so the description mentions the `add <description>` subcommand (e.g., "Scan deferred work and produce BACKLOG.md; use `add <desc>` to append an ad-hoc item.").
    **Verify:** `grep -q 'add <' docs/jim/WORKFLOW.md && grep -q '/jim:backlog' docs/jim/WORKFLOW.md`
