...
- **`docs/jim/specs/007-arch-feedback/spec.md:28-36`** — Data flow for the arch feedback pattern. The backlog feedback loop mirrors this: check if file exists → invoke skill → present changes → continue.
- **`docs/jim/specs/007-arch-feedback/plan.md:1-101`** — Implementation plan for the arch feedback loop. Single-task plan that modified only `skills/build/SKILL.md`.
...
- **`skills/roadmap/SKILL.md:31-34`** — Scans `docs/jim/specs/**/*.md` via Glob and Greps frontmatter `title:` fields. Closest existing pattern to the backlog skill's scanning needs.
- **`docs/jim/VISION.md:59-66`** — Non-Goals section. The backlog skill cross-references items against these to detect conflicts.
...
1. Check if `docs/jim/ARCHITECTURE.md` exists → invoke `/jim:arch` differential update
2. Report to user, ask to mark plan complete
3. STOP and wait for confirmation

The backlog step would slot in after step 1, before the completion report. Same conditional pattern: check if `docs/jim/BACKLOG.md` exists, invoke `/jim:backlog`, present changes. The build skill invokes `/jim:arch` directly (not via a hook or subagent) — the backlog invocation follows the same direct-invocation approach.
...
The roadmap skill scans specs via `Glob docs/jim/specs/**/*.md` + `Grep title:` for frontmatter. The backlog skill needs a broader scan:

| Source | Glob pattern | What to extract |
|--------|-------------|-----------------|
| Spec out-of-scope | `docs/jim/specs/*/spec.md` | `## Out of Scope` sections |
| Plan out-of-scope | `docs/jim/specs/*/plan.md` | `## Out of Scope` sections |
| Research deferrals | `docs/jim/specs/*/research.md` | Deferred/not-adopting notes (no standard heading) |
| Brainstorms | `docs/jim/brainstorms/*.md` | Uncaptured ideas not yet routed to specs |
| Notes | `docs/jim/notes/*.md` | Freeform deferred ideas |
| Roadmap Later | `docs/jim/ROADMAP.md` | `## Later` section |
| Vision Non-Goals | `docs/jim/VISION.md` | `## Non-Goals` (for conflict checking, not extraction) |
...
- **No security concerns.** The skill only reads markdown files within `docs/jim/` and writes one output file. No external data, no user input beyond approval, no Bash execution.
