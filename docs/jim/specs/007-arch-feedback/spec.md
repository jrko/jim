...
- [ ] If docs/jim/ARCHITECTURE.md does not exist, skip the check entirely and proceed to the completion report — nothing to go stale, and creating a first ARCHITECTURE.md is a separate decision
- [ ] If docs/jim/ARCHITECTURE.md exists, invoke `/jim:arch` in its standard differential update mode (codebase scan → compare to existing doc → summarize changes → present and wait for approval)
