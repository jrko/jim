---
spec: docs/jim/specs/003-pm-strategy/spec.md
type: feature
---
...
| `docs/jim/VISION.md` | Empty placeholder. The vision skill will write to this path. |
| `docs/jim/ROADMAP.md` | Empty placeholder. The roadmap skill will write to this path. |
| `docs/jim/ARCHITECTURE.md` | Empty placeholder. Vision skill reads this for context if it exists. |
| `docs/jim/WORKFLOW.md` | Defines artifact locations, command reference, rules of engagement. Skills reference key paths from here. |
...
| `docs/jim/prior-art/pm/pm-prior-art-research.md` | Detailed prior art study covering deanpeters, GSD, CCPM, SuperClaude. Primary reference for patterns. |
...
| 5 | 002-pm-core spec | Local spec | `docs/jim/specs/002-pm-core/spec.md` |
| 6 | 002-pm-core research | Local research | `docs/jim/specs/002-pm-core/research.md` |
| 7 | 002-pm-core plan | Local plan | `docs/jim/specs/002-pm-core/plan.md` |
| 8 | PM Prior Art Study Guide | Local research | `docs/jim/prior-art/pm/pm-prior-art-research.md` |
| 9 | V1 PM agent | Local prior art | `docs/jim/prior-art/V1SDLC/v1-agent-pm.md` |
| 10 | V1 PM skill | Local prior art | `docs/jim/prior-art/V1SDLC/v1-pm-skill.md` |
| 11 | 001-meta research | Local reference | `docs/jim/specs/001-meta/research.md` |
...
The PM agent body (from 002 plan `docs/jim/specs/002-pm-core/plan.md:279-303`) delegates all methodology to skills. It says "Follow the active skill's instructions." This means each 003 skill must be self-contained — the agent body provides identity and paths, skills provide process.
...
- `/jim:roadmap` reads VISION.md (if it exists) for strategic alignment, and Globs `docs/jim/specs/` to auto-link deliverables
...
| Progressive disclosure | `docs/jim/WORKFLOW.md:119-134` | SKILL.md ≤ 500 lines; templates in `assets/` |
...
- **Auto-link performance.** When `/jim:roadmap` Globs `docs/jim/specs/` to find linkable specs, use Glob (not Read) to list them. Only Read individual specs if linking is needed.
...
- Brainstorm filename convention (`docs/jim/brainstorms/{YYYYMMDD}-{topic}.md`)
...
5. **`docs/jim/brainstorms/` directory doesn't exist.** The brainstorm skill must create it on first use.
...
| **Roadmap auto-linking to specs may be noisy** | In a large project with many spec groups, Globbing `docs/jim/specs/` could produce too many matches. | Use Glob only — don't Read every spec. Present linked specs as suggestions, not requirements. The user decides which links to keep. |
...
4. **Roadmap auto-linking:** The skill should Glob `docs/jim/specs/` and present found specs as linkable candidates when the user mentions a deliverable that might have a spec. Don't auto-populate links — offer them.
