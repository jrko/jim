...
spec: docs/jim/specs/005-architect/spec.md
type: feature
status: draft
---
...
---
title: "{Title}"
spec: "docs/jim/specs/{group}/{id}-{name}/spec.md"  # relative path to source spec
...
5. [ ] Create `/jim:arch` skill at `skills/arch/SKILL.md` — frontmatter with `name: arch`, `agent: architect`, `argument-hint: "[directory-path]"`. Body covers: argument routing (empty → docs/jim/ARCHITECTURE.md, directory → subdirectory), read docs/jim/VISION.md as upstream context, codebase scanning (Glob + Grep to populate sections from actual code), differential update mode (existing ARCHITECTURE.md → read, summarize, Edit), generate from architecture-template.md, present and stop.
