...
- `docs/jim/specs/005-architect/spec.md:1-150`: Source spec defining `@jim:architect`, `/jim:plan`, and `/jim:arch`.
- `docs/jim/WORKFLOW.md:15-30`: The SDLC feedback loop — `/jim:plan` sits between `/jim:spec` and `/jim:build`, enforcing no ghost tasks.
...
- `docs/jim/ARCHITECTURE.md`: Empty — target output of `/jim:arch`, treated as locked constraint by `/jim:plan`.
