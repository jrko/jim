...
|----------|----------|------------|------------|
| Vision | `docs/jim/VISION.md` | Big picture — problem statement, value prop, target audience, competitive landscape | `/jim:vision` |
| Architecture | `docs/jim/ARCHITECTURE.md` | Technical foundation — codemap, system diagram, tech stack, data structures, architectural invariants | `/jim:arch` |
| Roadmap | `docs/jim/ROADMAP.md` | Execution sequence — milestones, phase breakdowns, links to numbered specs with status | `/jim:roadmap` |
...
> **Not yet implemented.** Jim does not currently support updating itself from within a host project. To develop jim, work directly in the jim repository. Jim's own docs live under `docs/jim/` within the jim repo, following the same structure as any project.
...
**Output:** `docs/jim/specs/{group}/{00X}-{name}/spec.md`
...
**Output:** `docs/jim/specs/{group}/{00X}-{name}/plan.md`
...
Jim's own specs live in `docs/jim/specs/` as a group. Jim's strategic docs (`VISION.md`, `ARCHITECTURE.md`, `ROADMAP.md`) live in `docs/jim/`.
